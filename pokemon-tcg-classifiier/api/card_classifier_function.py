...
class PokemonCardDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        if not os.path.exists('classes/classes.npy'):
            np.save('classes/classes.npy', self.label_encoder.classes_)
...
def get_model(model_name, model_path, num_classes, device):
    match model_name:
...
def setup_training(model_name, pretrain_path="pokemon_classifier.pth"):
    ...
    #Initialize Model
    model = get_model(model_name, pretrain_path, num_classes, device)
...
def train_model(model_name, pretrain_path = "model/pokemon_classifier.pth"):
    accuracy = 0 
    model, device, train_loader, test_loader, optimizer, criterion, scheduler = setup_training(model_name, pretrain_path)
    for epoch in range(1, 51):
        ...
        if accuracy >= 99.9:
            ...
            torch.save(model.state_dict(), "model/pokemon_classifier.pth")
            break
...
if __name__ == '__main__':
    accuracy = train_model(model_name="efficientnet_b0")
